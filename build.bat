...
rem del *.pdb
cl /LD ..\src\Gameh.cpp ..\src\GamePhysics.cpp ..\src\PlatformP.cpp ..\src\TileX.cpp ..\src\xTank.cpp ..\src\XTexture.cpp %COMMON_COMPILER_FLAG% /I %DEFAULT_INCLUDE_DIR% /I %SDL_INCLUDE_DIR% /link /PDB:main_%hr%%min%%sec%_%dd%%mm%%yr%.pdb %EXPORT_% /LIBPATH:%SDL_LIB_DIR% SDL2.lib SDL2_image.lib SDL2_mixer.lib SDL2_ttf.lib shlwapi.lib Shell32.lib /DEBUG


cl ..\src\*.cpp %COMMON_COMPILER_FLAG% /I %DEFAULT_INCLUDE_DIR% /I %SDL_INCLUDE_DIR% /link /PDB:main_%hr%%min%%sec%_%dd%%mm%%yr%.pdb %EXPORT_% /LIBPATH:%SDL_LIB_DIR% %INHERENT_LIB% /DEBUG /subsystem:console /OUT:%OUT_EXE%
